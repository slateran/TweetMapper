        ...
        try:
            ...
            if data['coordinates'] == None: #if there are no coordinates we do not want this tweet
               # print('**No Geotag for this Tweet**')
                ...
                c.execute(
                    "INSERT INTO suspicious (userid, username, tweet, language, location, longitude, latitude) VALUES (%s,%s,%s,%s,%s,%s,%s)",#sql statement to insert data collected \
...
def suspicious():
    twitterStream.filter(track=['suspicious', 'unwanted person'],async=True)
...
def Stoopingsus():
    ...
    sus()#function from map script to print tweet and launch browser showing map
