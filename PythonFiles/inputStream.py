        ...
        try:
            ...
            if data['coordinates'] == None: #if there are no coordinates we do not want this tweet
               # print('**No Geotag for this Tweet**')
                ...
                c.execute(
                    "INSERT INTO trump (userid, username, tweet, language, location, longitude, latitude) VALUES (%s,%s,%s,%s,%s,%s,%s)",#sql statement to insert data collected \
...
def userinput():
    user = input("Please enter a keyword to search for: ")
    twitterStream.filter(track=[(user)], async = True)
...
def Stoopingin():
    ...
    inp()#function from map script to print tweet and launch browser showing map
