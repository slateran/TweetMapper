        ...
        try:
            ...
            if data['coordinates'] == None: #if there are no coordinates we do not want this tweet
               # print('**No Geotag for this Tweet**')
                ...
                c.execute(
                    "INSERT INTO twitterdb (userid, username, tweet, language, location, longitude, latitude) VALUES (%s,%s,%s,%s,%s,%s,%s)",#sql statement to insert data collected \
...
def theft():
    twitterStream.filter(track=['theft', 'stolen'], async=True) #keyword filtered search of twitter stream

def Stooping():
    twitterStream.disconnect() #when called this will break the connection with the twitter stream
    tmap() #function from map script to print tweet and launch browser showing map
